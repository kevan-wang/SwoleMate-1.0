...
//  SwoleMate 1.0
//
//  Created by Kevan Wang on 9/26/18.
//  Copyright © 2018 Kevan Wang. All rights reserved.
//

import UIKit

class NewPlan4VC: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @IBAction func saveButtonPressed(_ sender: UIButton) {
        ...
        DBManager.shared.newPlanDict["auxSetType"] = setType
        let regimenFormat = regimenFormatString()
        DBManager.shared.newPlanDict["auxRegimenFormat"] = regimenFormat
        let regimenTypeNum = PlanBuilder.shared.myRegimen["Regimen"]
        if regimenTypeNum == 0 {
            DBManager.shared.newPlanDict["routineType"] = "Split"
        } else {
            DBManager.shared.newPlanDict["routineType"] = "Full Body"
        }
        DBManager.shared.createFitnessPlan()
        let rootVC = self.presentingViewController?.presentingViewController?.presentingViewController?.presentingViewController
        //  rootVC.reloadTable()
        rootVC?.dismiss(animated: true, completion: nil)
//        print(DBManager.shared.newPlanDict)
//        performSegue(withIdentifier: "NewPlanSegue5", sender: sender)
    }
    
    
    //  INFO BUTTON FUNCTIONS
    
    @IBAction func auxExInfo(_ sender: UIButton) {
        let title = "Auxiliary Exercises"
        let message = "\nAuxiliary Exercises are meant to exercise the target muscle at the stretch or contracted positions of flexion.  These help produce a synergistic response that increase your gains!\n\nSwoleMate recommends a default of 2 standard sets for Auxiliary exercises!"
    ...
    func setDefaultValues() {
        ...
        setTypePicker.selectRow(0, inComponent: 0, animated: false)
        setRepsSets()
        pickerTextUpdate()
    }
    ...
    func generateRegimenDesc() -> String {
        ...
        var str = "\n\nYour auxiliary exercise rep-set format is currently "
